...
export interface SourceSummary { readonly kind: 'source'; readonly id: SourceId; readonly provider: ProviderId; readonly recordingClass: RecordingClassSummary; readonly label: string; readonly capabilities: readonly Capability[] }
...
export function queryCatalog(query: CatalogQuery = {}): CatalogQueryResult {
  ...
  for (const leaf of catalog) {
    ...
    for (const source of matchingSources) sources.push({ kind: 'source', id: source.id as SourceId, provider: source.provider as ProviderId, recordingClass: summary, label: source.label, capabilities: PROVIDER_CAPABILITIES[source.provider as ProviderId] })
  }
